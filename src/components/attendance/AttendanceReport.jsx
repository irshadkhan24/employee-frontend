...
const AttendanceReport = () => {
  const [report, setReport] = useState({})  // ✅ initialized as object
  ...
  const fetchReport = async () => {
    try {
      ...
      console.log(response)  // ✅ corrected variable name
      if (response.data.success) {
        if (skip == 0) {
          setReport(response.data.groupData || {}) // ✅ fallback to {}
        } else {
  ...
  return (
    <div className='min-h-screen p-10 bg-white'>
      ...
      <button className="px-4 py-2 border bg-gray-100 hover:bg-gray-200 text-lg font-semibold" onClick={handleLoadmore}>Load More</button>
    </div>
  )
}
