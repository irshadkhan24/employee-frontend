...
const Detail = () => {
  ...
  const changeStatus = async (id, status) => {
    try {
      const responnse = await axios.put(`http://localhost:5000/api/leave/${id}`, {status}, {
        headers: {
          Authorization : `Bearer ${localStorage.getItem('token')}`
        },
      })
    ...
    <div className='max-w-4xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
          ...
          <div className="flex space-x-4">
             <button
                className="px-4 py-0.5 bg-teal-300 hover:bg-teal-400 transition"
                onClick={() => changeStatus(leave._id, "Approved")}>Approve</button>
             <button
                className="px-4 py-0.5 bg-red-300 hover:bg-red-400 transition"
